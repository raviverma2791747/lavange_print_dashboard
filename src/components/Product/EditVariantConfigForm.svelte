...
<div class="border rounded-lg p-4">
  <div class="mb-4">
    ...
    <Select.Root
      disabled={create}
      selected={{
        value: variantConfigClone.status,
        label: getByValue(STATUS, variantConfigClone.status),
      }}
      onSelectedChange={(v) => {
        v && (variantConfigClone.status = v.value);
      }}
    >
      <Select.Trigger>
      ...
      {:else}
        <VariantSchemaOption
          ...
          disabledMoveDown={index ===
            variantConfigClone.variantSchema.length - 1 ||
            create ||
            edit_variant_schema_option_index !== -1}
